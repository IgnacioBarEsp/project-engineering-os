...
Review of the bounded-reader follow-up then identified a stable special-file edge case: opening a FIFO before checking its type can wait indefinitely. The reader now rejects non-regular paths before opening and uses nonblocking open on POSIX as an additional guard if a path changes to a FIFO during the check/open interval. A regression test verifies that a directory is rejected as non-regular. This was fixed before the final security scan; the final scan must cover the resulting complete commit range.
