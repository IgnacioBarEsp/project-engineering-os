...
test('the 0.3.3 release path is private, disposable and never replaces a prior release', async () => {
  const manifest = JSON.parse(await read('package.json'));
  const lock = JSON.parse(await read('package-lock.json'));
  ...
  const notes = await read('RELEASE_NOTES_0.3.3.md');
  ...
  assert.match(installEvidence, /candidate\.manifest\.version, '0\.3\.3'/);
