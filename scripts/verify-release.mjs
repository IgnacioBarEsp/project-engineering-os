...
export async function verifyRelease(releaseRoot, { expectedCommit } = {}) {
  ...
  if (manifest.bytes !== tarball.byteLength) {
    throw new Error('El tamaño comprimido del manifest no coincide con el tarball.');
  }
  ...
  if (!Number.isSafeInteger(manifest.fileCount) || manifest.fileCount < 1 || manifest.fileCount > 10_000
    || !Number.isSafeInteger(manifest.unpackedBytes) || manifest.unpackedBytes < 1
    || manifest.unpackedBytes > 128 * 1024 * 1024) {
    throw new Error('El manifest no contiene métricas válidas del inventario del paquete.');
  }
  return manifest;
}
